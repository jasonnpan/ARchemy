...
let InteractableSnap3DGenerator = class InteractableSnap3DGenerator extends BaseScriptComponent {
    ...
    createElementButton(element, index) {
        ...
        try {
            ...
            let pinchButton = null;
            try {
                // Try using getTypeName() - this should work
                pinchButton = buttonObject.getComponent(PinchButton_1.PinchButton.getTypeName());
    ...
    checkButtonChildren(buttonObject, element) {
        try {
            ...
            try {
                const children = pinchButtonSceneObject.children;
                ...
                if (children.length > 0) {
                    ...
                    try {
                        const textComponent = firstChild.getComponent("Text");
                        if (textComponent) {
                    ...
                    try {
                        ...
                        allComponents.forEach((comp, compIndex) => {
                            ...
                            if (comp.text !== undefined) {
                                print(`     -> Has text property: "${comp.text}"`);
                                // Try to update the text
                                comp.text = element.toUpperCase();
